...
{
    public class AfipHomoLog
    {
        private Funciones func = new Funciones();
        private Controles ctrls = new Controles();
        private static UInt32 _globalId = 0;
        private static string urlLoginCms = "https://wsaahomo.afip.gov.ar/ws/services/LoginCms?wsdl";
        public string urlWSDL { get; set; } = "https://wswhomo.afip.gov.ar/wsfev1/service.asmx?wsdl";
        ...
        public AfipHomoLog(string cert_path, string clave)
        {
        ...
        public void HacerLogin(long cuit)
        {
            ...
            try
            {
                ...
                AMDGOINT.AFIP.HOMO.WSAA.LoginCMSService servicio = new AMDGOINT.AFIP.HOMO.WSAA.LoginCMSService();
        ...
        private void guardaTa(long cuit)
        {
            try
            {
        ...
        private bool taValido(long cuit)
        {
            try
            {                
                bool retorno = false;
                ...
                string consulta = "SELECT TOP 1 * FROM WSFEACCESO" +
                    " WHERE Tipo = 0 AND ExpirationTime > SYSDATETIME() AND Cuit = " + cuit;

                foreach (DataRow fila in func.getColecciondatos(consulta, SqlConnection.State == ConnectionState.Open ? SqlConnection : null).Rows)
                {                    
