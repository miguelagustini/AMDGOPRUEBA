...
namespace AMDGOINT.Formularios.Caja.Vista
{
    public partial class Frm_MuestraAsiento : XtraForm
    {
        ...
        public List<MC.CajaMovimientos> MovimientosCaja { get; set; } = new List<MC.CajaMovimientos>();
        ...
        private void BusqDetalles()
        {
            ...
            {
                CargaCombos();
                gridControl.DataSource = new BindingList<MC.CajaMovimientos>(MovimientosCaja); ;
            }
            catch (Exception e)
            {
                ctrl.MensajeInfo($"Hubo un inconveniente al cargar la lista.\n{e.Message}", 1);
        ...
        private void BorrarAsiento()
        {
            try
            {
                MC.CajaMovimientos _tmp = gridView.GetRow(gridView.FocusedRowHandle) as MC.CajaMovimientos;
            ...
            try
            {
                if (e.Column == colDebe || e.Column == colHaber)
                {
                    ...
                    */
                    ...
                    if (e.Column == colHaber && MovimientosCaja.Sum(s => s.ImporteDebe) > MovimientosCaja.Sum(s => s.ImporteHaber))
                    {
                        e.Cache.DrawRectangle(e.Cache.GetPen(Color.FromArgb(255, 128, 128), 5), e.Bounds);

                    }
                    else if (e.Column == colDebe && MovimientosCaja.Sum(s => s.ImporteDebe) < MovimientosCaja.Sum(s => s.ImporteHaber))
                    { e.Cache.DrawRectangle(e.Cache.GetPen(Color.FromArgb(255, 128, 128), 5), e.Bounds); }
                    else
                    {
