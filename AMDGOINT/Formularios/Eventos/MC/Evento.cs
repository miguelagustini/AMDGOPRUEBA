...
namespace AMDGOINT.Formularios.Eventos.MC
{
    public class Evento : INotifyPropertyChanged
    {
        ...
        private static string tablaname = "[AmdgoInterno].[dbo].[EVENTOS]";
        private string _descripcion = "";

        public short IDRegistro { get; set; } = 0;                
        public string Descripcion { get { return _descripcion; } set { _descripcion = value?.Trim(); } }     
        ...
        public Evento(SqlConnection conexion)
        {
        ...
        public Evento Clone()
        {
            Evento r = (Evento)MemberwiseClone();
            return r;
        }

        //CLONE CON LISTAS
        public List<Evento> Clone(List<Evento> lst)
        {
            List<Evento> lista = new List<Evento>();

            foreach (Evento d in lst)
            {
                lista.Add(d.Clone());
            }
        ...
        public List<Evento> GetRegistros()
        {
            try
            {
                ...
                string c = $"SELECT RE.IDRegistro, RE.Descripcion, RE.Origen FROM {tablaname} RE";
                ...
                using (SqlCommand cmd = new SqlCommand(c, SqlConnection.State != ConnectionState.Open ? cnb.Conectar() : SqlConnection))
                {
                    ...
                    lista.AddRange(func.ConvertToList<Evento>(rdr));
                }
        ...
        public bool ExisteRegistroDescripcion()
        {
            try
            {
                bool retorno = false;
                ConexionBD cnb = new ConexionBD();
                string c = $"SELECT IDRegistro FROM {tablaname} WHERE Descripcion = '{Descripcion}' AND IDRegistro <> {IDRegistro} AND Origen <> {Origen}";

                if (func.getColecciondatos(c, SqlConnection.State != ConnectionState.Open ? cnb.Conectar() : SqlConnection).Rows.Count > 0)
                {
                    retorno = true;
                }
            ...
            {
                ctrl.MensajeInfo($"Hubo un inconveniente al consultar la existencia del evento.\n{e.Message}", 1);
                return false;
            }
        }
