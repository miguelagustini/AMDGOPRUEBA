...
namespace AMDGOINT.Formularios.Facturas.Prestatarias.MC
{
    public class ComprobantesRel
    {
        ...
        private static string tablaname = "FACTPRESREL";
        private static string tablacomprobante = "FACTPRESENC";
        ...
        public List<ComprobantesRel> GetRegistros()
        {
            try
            {
                ...
                string c = $"SELECT T1.ID_Registro AS IDRegistro, T1.ID_NotaCredito AS IDNotaCredito, T1.ID_NotaDebito AS IDNotaDebito," +
                           $" T1.ID_Factura AS IDFactura, T1.Comprobante AS ComprobanteManual, " +
                           ...
                           $" FROM {tablaname} T1" +
                           $" LEFT OUTER JOIN {tablacomprobante} NC ON(NC.ID_Registro = T1.ID_NotaCredito AND NC.EstadoAf = 'A')" +
                           $" LEFT OUTER JOIN {tablacomprobante} ND ON(ND.ID_Registro = T1.ID_NotaDebito AND ND.EstadoAf = 'A')";
                ...
                using (SqlCommand cmd = new SqlCommand(c, SqlConnection.State == ConnectionState.Open ? SqlConnection : cnn.Conectar()))
                {
                    ...
                    lista.AddRange(func.ConvertToList<ComprobantesRel>(rdr));
                }
        ...
        public List<ComprobantesRel> GetRegistrosPorComprobante(long idcomprobante)
        {
            try
            {
                ...
                string c = $"SELECT T1.ID_Registro AS IDRegistro, T1.ID_NotaCredito AS IDNotaCredito, T1.ID_NotaDebito AS IDNotaDebito," +
                           ...
                           $" LEFT OUTER JOIN {tablacomprobante} NC ON(NC.ID_Registro = T1.ID_NotaCredito AND NC.EstadoAf = 'A')" +
                           $" LEFT OUTER JOIN {tablacomprobante} ND ON(ND.ID_Registro = T1.ID_NotaDebito AND ND.EstadoAf = 'A')" +
                           $" WHERE T1.ID_Factura = {idcomprobante}";
