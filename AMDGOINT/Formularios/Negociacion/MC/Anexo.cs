...
namespace AMDGOINT.Formularios.Negociacion.MC
{
    public class Anexo: INotifyPropertyChanged
    {
        ...
        private static string tablaname = "XDISCUSIONOTROS";
        ...
        private string _practicacodigoam = "";
        ...
        public string PracticaCodigoPrestadora
        {
            get { return _practicacodigoos; }
            set { _practicacodigoos = _practicacodigoos != value.Trim() ? value.Trim() : _practicacodigoos; }
        }

        public string PracticaCodigoAmdgo
        {
            get { return _practicacodigoam; }
            set { _practicacodigoam = _practicacodigoam != value.Trim() ? value.Trim() : _practicacodigoam; }
        }

        public string Importe
        {
            get { return _importe; }
            set { _importe = _importe != value.Trim() ? value.Trim() : _importe; }
        }
        ...
        public Anexo(SqlConnection conexion)
        {
        ...
        public Anexo Clone()
        {
            Anexo d = (Anexo)MemberwiseClone();
            return d;
        }

        //CLONE CON LISTAS
        public List<Anexo> Clone(List<Anexo> lst)
        {
            List<Anexo> lista = new List<Anexo>();

            foreach (Anexo d in lst)
            {
                lista.Add(d.Clone());
            }
        ...
        public List<Anexo> GetRegistros()
        {
            try
            {
                ...
                string c = $"SELECT EX.ID_Registro AS IDRegistro, EX.ID_Encabezado AS IDEncabezado, EX.CodigoOs AS PracticaCodigoPrestadora, EX.CodigoAm AS PracticaCodigoAmdgo," +
                           $" EX.Descripcion, EX.Observacion, EX.Valor AS Importe" +
                           $" FROM {tablaname} EX";
                ...
                using (SqlCommand cmd = new SqlCommand(c, SqlConnection.State != ConnectionState.Open ? cnb.Conectar() : SqlConnection))
                {
                    ...
                    lista.AddRange(func.ConvertToList<Anexo>(rdr));
                }
