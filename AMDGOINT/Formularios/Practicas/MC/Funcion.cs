...
{
    public class Funcion : INotifyPropertyChanged
    {
        ...
        // PROPIEDADES 
        #region PROPIEDADES        
        private static string tablaname = "FUNCIONES";
        ...
        public string Letra
        {
            ...
            set
            {
                _letra = _letra != value.Trim() ? value.Trim() : _letra;
            }
        }
        ...
        public Funcion(SqlConnection conexion) { SqlConnection = conexion; }
        ...
        {
            Funcion d = (Funcion)MemberwiseClone();
            return d;

        }

        //CLONE CON LISTAS
        public List<Funcion> Clone(List<Funcion> lst)
        {
            List<Funcion> lista = new List<Funcion>();

            foreach (Funcion d in lst)
            {
                lista.Add(d.Clone());
            }
        ...
        public List<Funcion> GetRegistros()
        {
            try
            {
                List<Funcion> lista = new List<Funcion>();

                string c = $"SELECT ID_Registro AS IDRegistro, Descripcion, Codigo, Letra" +
                           $" FROM {tablaname}";
            ...
            {
                ctrl.MensajeInfo($"Ocurrió un error al consultar las funciones.\n{e.Message}", 0);
