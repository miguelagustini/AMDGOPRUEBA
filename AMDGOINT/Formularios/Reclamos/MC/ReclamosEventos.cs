...
{
    public class ReclamosEventos : INotifyPropertyChanged
    {
        ...
        private static string tablaname = "[AmdgoInterno].[dbo].[RECLAMOSEVENTOS]";
        
        public int IDRegistro { get; set; } = 0;
        public long IDEncabezado { get; set; } = 0;
        public short IDEvento { get; set; } = 0;
        public string EventoDescripcion { get; set; } = "";
        public byte Responsable { get; set; } = 0;
        public string ResponsableDescripcion { get { return Responsable == 0 ? "Personal Am" : Responsable == 1 ? "Prestadora" : "Profesional"; } }
        ...
        //CONSTRUCTOR VACIO
        public ReclamosEventos() { }

        public ReclamosEventos(SqlConnection conexion)
        {
        ...
        //CLONE 
        public ReclamosEventos Clone()
        {
            ReclamosEventos r = (ReclamosEventos)MemberwiseClone();
            return r;
        }

        //CLONE CON LISTAS
        public List<ReclamosEventos> Clone(List<ReclamosEventos> lst)
        {
            List<ReclamosEventos> lista = new List<ReclamosEventos>();

            foreach (ReclamosEventos d in lst)
            {
                lista.Add(d.Clone());
            }
        ...
        public List<ReclamosEventos> GetRegistros()
        {
            try
            {
                ...
                string c = $"SELECT RE.IDRegistro, RE.IDEncabezado, RE.IDEvento, RE.Fecha, RE.Observacion, RE.Responsable FROM {tablaname} RE";
                ...
                using (SqlCommand cmd = new SqlCommand(c, SqlConnection.State != ConnectionState.Open ? cnb.Conectar() : SqlConnection))
                {
                    ...
                    lista.AddRange(func.ConvertToList<ReclamosEventos>(rdr));
                }
            ...
            catch (Exception e)
            {
                ...
                return new List<ReclamosEventos>();
            }
        }
